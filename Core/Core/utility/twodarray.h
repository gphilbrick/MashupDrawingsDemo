...
public:
    ...
    explicit TwoDArray( TwoDArray&& other )
    {
        ...
        _array = other._array;
        other._array = nullptr; // Or destructor will kill the block.
        other.recreate( 1, 1 );
    }
    ...
    T get(int x, int y) const
    {
        ...
        if(!isValidCoord(x,y))
        {
            throw std::runtime_error( "illegal get attempted!" );
        }
#endif
    ...
    void set(int x, int y, T val)
    {
        ...
        if(!isValidCoord(x,y))
        {
            throw std::runtime_error( "Illegal set attempted!" );
        }
#endif
    ...
    {
        if( width < 1 || height < 1 ) throw std::runtime_error( "Illegal dimensions" );
        _width=width;
        _height=height;
    ...
    void drawLine( const IntCoord& a, const IntCoord& b, T val )
    {
        const auto coords = rasterizeSegment_midpoint( a, b );
...
private:
    ...
    //forbid
    TwoDArray(const TwoDArray& other); //forbid
    TwoDArray& operator=(const TwoDArray& other); //forbid

    int _width;
    int _height;

    //contains entries in row major order
    T* _array;
};
