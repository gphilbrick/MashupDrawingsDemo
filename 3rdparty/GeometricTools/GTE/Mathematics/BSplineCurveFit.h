...
namespace gte
{
    ...
    public:
		...
		//
		// Greg Philbrick addition: 'sampleTimes' must be other null or point to 'numSamples' T values in [0,1]
		// corresponding to the points addressed by 'sampleData'.
        BSplineCurveFit(
			int32_t dimension, int32_t numSamples, Real const* sampleData,
            int32_t degree, int32_t numControls,
			Real const* sampleTimes )
            :
            ...
            mControlData(static_cast<size_t>(dimension) * static_cast<size_t>(numControls))
        {
            ...
            for (i0 = 0; i0 < mNumControls; ++i0)
            {
                ...
                for (i1 = i0; i1 <= i1Max; ++i1)
                {
                    ...
                    {
                        t = sampleTimes ? sampleTimes[ i2 ] : tMultiplier * (Real)i2;
            ...
            for (i0 = 0; i0 < mNumControls; ++i0)
            {
                ...
                {
                    t = sampleTimes ? sampleTimes[ i1 ] : tMultiplier * (Real)i1;
                    mBasis.Evaluate(t, 0, imin, imax);
                    if (imin <= i0 && i0 <= imax)
                    {
